/*************************************************************************************
* Obstacle detection from moving vehicle using disparity image + pedestrian detection HOG +SVM
 * Author: Álvaro Gregorio Gómez
 * Date: 15/9/2016
 * Este codigo procesa una secuencia de imágenes de tamaño N_FRAMES ubicadas en Video/Secuencias/left & Video/Secuencias/right
 *************************************************************************************/
...
// Parámetros del SGBM(Semi-Global Block matching)
	...
	/***********************************************************************
	 * Tratamiento de la u-disparity para identificar obstáculos y edificios
	 ***********************************************************************/
    ...
    /**************************************************************
     * Detección de edificios por Hough lines oblicuas en u-disparity (deshabilitado)
     **************************************************************/
    ...
    for( size_t i = 0; (i < lines_obstacles.size() && indice<30); i++ ){
    	if((lines_obstacles[i][1] == lines_obstacles[i][3]) && (lines_obstacles[i]!=(Vec4i)Scalar(0,0,0,0))){
    		...
    		//Comprobación de que están por debajo de las lineas de la carretera y no detecta por error una linea en y=0 (Aunque deje alguna linea del fondo sin detectar no importa (DESHABILITADO)
...
int main(int argc, char** argv){
	...
	// Codigo para procesar secuencia de imágenes de tamaño N_FRAMES ubicadas en Video/Secuencias/left & Video/Secuencias/right
		...
		for(i=0;i<=N_FRAMES;i++){
			...
			image_name_left=path+"Secuencias/left/"+(string)image_number+".png";
			image_name_right=path+"Secuencias/right/"+(string)image_number+".png";
